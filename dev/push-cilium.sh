...
cd ..
#make
gcloud compute ssh $INSTANCE --command "sudo killall cilium-agent" -- -o ProxyCommand='corp-ssh-helper %h %p'
gcloud compute scp ./daemon/cilium-agent $INSTANCE:~/ --scp-flag "-o ProxyCommand=/usr/bin/corp-ssh-helper %h %p"
gcloud compute ssh $INSTANCE --command "\
	...
	> cilium-agent.stdout 2> cilium-agent.stderr < /dev/null &" -- -o ProxyCommand='corp-ssh-helper %h %p'
