...
// EgressPolicyConfig is the internal representation of Cilium Egress NAT Policy.
...
func (config *EgressPolicyConfig) policyConfigSelectsEndpoint(endpointInfo *endpointMetadata) bool {
	log.WithFields(logrus.Fields{"policyID": config.id.Name, "endpointname": endpointInfo.id.Name}).Debug("=========matching policy with endpoint")
...
// Parse takes a CiliumEgressNATPolicy CR and converts to EgressPolicyConfig, the internal
// representation of the egress nat policy
func Parse(cenp *v2alpha1.CiliumEgressNATPolicy) (*EgressPolicyConfig, error) {
	...
	for _, cidrString := range cenp.Spec.DestinationCIDRs {
		...
		if err != nil {
			log.WithError(err).Warnf("CiliumEgressNATPolicy %v error parsing cidr", name)
			continue
		}
	...
	for _, egressRule := range cenp.Spec.Egress {
		if egressRule.NamespaceSelector != nil {
			...
			return nil, fmt.Errorf(
				"CiliumEgressNATPolicy %v cannot have both nil namespace selector and nil endpoint selector",
				name)
		}
	}
