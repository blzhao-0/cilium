...
func validateCiliumSvc(cSvc models.Service, k8sSvcs []v1.Service, k8sEps []v1.Endpoints, k8sServicesFound map[string]bool) error {
	...
	if cSvc.Status.Realized.Flags != nil {
		...
		if typ == models.ServiceSpecFlagsTypeHostPort {
...
K8S_SVCS:
	for _, k8sSvc := range k8sSvcs {
		...
		case "LoadBalancer":
			for _, svcPort := range k8sSvc.Spec.Ports {
				if svcPort.NodePort == int32(cSvc.Status.Realized.FrontendAddress.Port) {
					return fmt.Errorf("Found Cilium service of type %v with address %s:%d in k8s NodePort",
				...
				if svcPort.Port == int32(cSvc.Status.Realized.FrontendAddress.Port) {
					return fmt.Errorf("Found Cilium service of type %v with address %s:%d in k8s Port",
						typ, cSvc.Spec.FrontendAddress.IP, cSvc.Spec.FrontendAddress.Port)
				}
			}
			if string(k8sSvc.Spec.Type) == typ {
				return fmt.Errorf("Found Cilium service of type %v with address %s:%d in k8s ",
					typ, cSvc.Spec.FrontendAddress.IP, cSvc.Spec.FrontendAddress.Port)
			}

			if k8sSvc.Spec.LoadBalancerIP == cSvc.Status.Realized.FrontendAddress.IP {
				return fmt.Errorf("Found Cilium service of type %v with address %s:%d in k8s LoadBalancerIP",
					typ, cSvc.Spec.FrontendAddress.IP, cSvc.Spec.FrontendAddress.Port)
			}
			if k8sSvc.Spec.ClusterIP == cSvc.Status.Realized.FrontendAddress.IP {
				return fmt.Errorf("Found Cilium service of type %v with address %s:%d in k8s ClusterIP",
					typ, cSvc.Spec.FrontendAddress.IP, cSvc.Spec.FrontendAddress.Port)
			}
			// exception - LoadBalancer is stored as ExternalIP
			for _, svcIP := range k8sSvc.Spec.ExternalIPs {
				if svcIP == cSvc.Status.Realized.FrontendAddress.IP {
					return fmt.Errorf("Found Cilium service of type %v with address %s:%d in k8s ExternalIPs",
						typ, cSvc.Spec.FrontendAddress.IP, cSvc.Spec.FrontendAddress.Port)
				}
			}
		default: // ClusterIP
